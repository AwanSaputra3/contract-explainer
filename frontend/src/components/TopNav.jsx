...
export default function TopNav({ isDashboard, onMenuClick }) {
  ...
  return (
    <nav className={`fixed top-0 w-full z-40 bg-surface/80 backdrop-blur-xl border-b border-outline-variant/30 shadow-sm`}>
      <div className={`flex justify-between items-center px-lg py-sm w-full ${!isDashboard ? 'max-w-container-max mx-auto' : ''}`}>
        ...
        {!isDashboard ? (
          ...
          <>
            <button onClick={onMenuClick} className="text-on-surface-variant hover:text-primary transition-colors p-sm -ml-sm rounded-lg hover:bg-surface-variant/20 flex items-center justify-center">
              <span className="material-symbols-outlined text-[24px]">menu</span>
            </button>
            <div className="font-headline-md text-headline-md text-on-surface ml-sm">
              Dashboard Overview
            </div>
          </>
        )}
      ...
      <div className="flex items-center gap-sm">
        ...
        {isDashboard && (
          <div className="flex items-center gap-sm">
            ...
            <button onClick={handleLogout} className="text-error hover:bg-error/10 p-sm rounded-lg transition-colors flex items-center justify-center" title="Logout">
